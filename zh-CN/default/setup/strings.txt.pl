...
这代表你并没有先 drop 已经存在的 Bugzilla 数据库，就把 mysqldump
回存到 Bugzilla 数据库。当你需要回存 Bugzilla 数据库的备份时，
你必须先 drop 整个数据库。

请 drop 你的 Bugzilla 数据库，并从一个不包含 bz_schema table 的备份回存。
如果因为某些原因你无法这样做，那么你可以使用最后的手段：连到你的 MySQL
数据库后， drop bz_schema table 。
END
    checking_for => '检查',
...
END
    ...
    feature_jobqueue => '邮件队列',
    feature_jsonrpc => 'JSON-RPC 接口',
...
设置为 0 时， checksetup.pl 将不会建立 .htaccess 文件。
END
    localconfig_db_check => <<'END',
checksetup.pl 是否应该尝试检查你的数据库设置是否正确？
有些数据库服务器/Perl 模块/moonphase 的组合会让此功能无法运作，
因此你可以尝试设置此参数为 0 ，让 checksetup.pl 可以执行。
END
    localconfig_db_driver => <<'END',
使用哪一种 SQL 数据库。默认值是 mysql 。
支持的数据库列表可以借由列出 Bugzilla/DB 目录维护 －－
每个模块都对应到一种支持的数据库，而模块的档名（在 ".pm" 之前）
都对应到此参数的一个有效的值。
END
...
有时数据库服务器会使用非标準的 port 。如果有这种情况的话，在此参数指定该
...
以避免重导到 index.cgi 。设置 $index_html 为 1 时，
...
如果设置为 1 ， checksetup.p 会设置适合的文件权限，让 Bugzilla 在
...
你可以在 MySQL 设置档中的 [mysqld] 段落中找到此参数。
EOT
...
* 這些模组可以在 Apache 的设置档（通常叫 httpd.conf 或 apache2.conf ）*
* 中启用。                                                            *
...
* - mod_version:                                                      *
*   可以针对 Apache 2.2 或 2.4 写入规则至 .htaccess 文件中。          *
* 你需要启用的模组有：                                                *
*                                                                     *
END
...
* "DBD" 模块。请在以下指令中找出正确的指令，以在数据库中安装适当      *
* 的模块。                                                            *
EOT
...
警告：我们将把你所保存的资料格式转成 UTF-8 。这将会允许 Bugzilla 正确地
      保存及排序国际字符。然而，如果你的数据库中有任何非 UTF-8 的资料，
      ...
      资料转成 UTF-8 。你在继续之前也应该先备份。将会影响数据库中的所有
...
END
    no_such_module => "在 CPAN 上找不到叫 ##module## 的 Perl 模组。",
...
警告：无法移除目录 '##template_cache##' 。
      ...
      请手动删除。
END
    template_removing_dir => "删除存在的已編譯模版中...",
